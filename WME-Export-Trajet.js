...
(function main() {
    ...
    function log(msg, obj) {
        if (obj === undefined) { console.log(`${SCRIPT_NAME} v${SCRIPT_VERSION} - ${msg}`); } else if (debug) { console.debug(`${SCRIPT_NAME} v${SCRIPT_VERSION} - ${msg} `, obj); }
    }
    ...
    function WMEExpT_init() {
        ...
        if (I18n.locale === 'fr') {
            ...
            lang = ['Export', 'Select the Drive', 'Time', 'Speed'];
        }

        userName = W.loginManager.user.getUsername();
    ...
    function WMEExpT_html() {
        ...
        getId('gpx').onclick = function onGpxClick() { if (getId('kml').checked) getId('kml').checked = false; generateTypeExport(); };
        getId('kml').onclick = function onKmlClick() { if (getId('gpx').checked) getId('gpx').checked = false; generateTypeExport(); };
        generateTypeExport();
        log('html Ok');
    }
    ...
    function updateTrajetsList() {
        ...
        for (let i = 0; i < available_Trajets.length; i++) {
            ...
            const temp = trajetDetail.tps.split(' ');
            log('trajetDetail.tps.split', temp);
            const tps = [];
            for (let l = 0; l < temp.length; l++) {
                if (temp[l].match(/^[0-9]{1,10}/g) != null) { tps.push(temp[l]); }
            }
            ...
            const expTrOption = document.createElement('option');
            const expTrText = document.createTextNode(WMEExpT.Trajets.list[trajetID].text);
    ...
    function ExpTrajet() {
        ...
        if (selectTrajet.selectedIndex <= 0) { return; }

        const { id } = selectTrajet.options[selectTrajet.selectedIndex];

        if (id <= 0) { return; }

        if (!WMEExpT.Trajets.objects.hasOwnProperty(id)) {
            let url = document.location.origin;
            // TODO: update for other servers??? (NA, IL)
            ...
            requestObject.onreadystatechange = function onreadystatechange() {
    ...
    */
        ...
        const nameIti = `<name>${roadName[0]} vers ${roadName[1]}</name>\n<trkseg>\n`;
    ...
    function generateKmlFile(id) {
        ...
        log(`${roadName[0]} vers ${roadName[1]}`);
        const nameIti = `<Folder><name>${roadName[0]} vers ${roadName[1]}</name>\n`;
